...
<<<<<<< HEAD
          <img :src="item.goodsPhoto" alt=""  @click="detail(item)">
          <div>
...
<<<<<<< HEAD
    ...
    //     if (this.list[i].checked === true) {
    //       req('saveOrder', {
    //         goodsCode: this.list[i].goodsCode,
    //         sellPrice: this.list[i].sellPrice,
    //         goodsCount: this.goods.goodsCount,
    //         orderTotal: parseInt(this.list[i].goodsCount * this.list[i].sellPrice),
    //         cartCode: this.list[i].cartCode,
    ...
    settlement () {
      if (this.role.shopCode === undefined) {
        ...
        let sellPrice = this.list.filter(item => {
          return item.checked === true
        }).map(item => {
          return item.sellPrice
        }).toString()

        let goodsCount = this.list.filter(item => {
          return item.checked === true
        }).map(item => {
          return item.goodsCount
        }).toString()

        let cartCode = this.list.filter(item => {
          return item.checked === true
        }).map(item => {
        ...
        req('saveOrder', {
          ...
          // 全局变量
          shopCode: JSON.parse(sessionStorage.getItem('roleInfo')).shopCode
        }).then(data => {
          if (data.code === 0) {
            this.$message.success(data.msg)
